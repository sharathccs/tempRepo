...
<xsl:stylesheet version="2.0"
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
               <xsl:call-template name="overHead"/>
                <body>
				<center>
                    <h1>VMware Smoke Suite Report on <xsl:value-of  select="current-date()"/>
					</h1>
					</center>
                    <table width="100%">
                        <tr>
    ...
    <xsl:template name="testMethodsList">
        ...
        <xsl:for-each select="$methodList">
			...
			<xsl:variable name="CaseName" select="tokenize($ClassName,'\.')[last()]"/>
			<xsl:variable name="TokenCount" select="count(tokenize($ClassName,'\.'))"/>
			<xsl:variable name="ReportHTML" select="concat($CaseName,'.html')"/>
